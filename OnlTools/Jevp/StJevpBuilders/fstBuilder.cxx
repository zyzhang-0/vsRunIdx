...
void fstBuilder::event(daqReader *rdr) 
{
  ...
  //    int OfflineId_tmp = rdr->getOfflineId(i);
  //    if(OfflineId_tmp==900315) fSkip = 0;
  //  }
  //}
  //fSkip = 0; //ZIYUE switch
  //if(fSkip==1) return;
  //if(trgd) delete trgd;
...
void fstBuilder::stoprun(daqReader *rdr) 
{
  ...
  */
  ...
  for(int i = 0 ; i < 3 ; i++){
    hFracVsApvIdx[i] = new TH1F(Form("hFracVsApvIdx_%d", i), Form("hFracVsApvIdx_%d", i), 288, -0.5, 287.5);
  }
  hMeanVsApvIdx = new TH1F("hMeanVsApvIdx", "hMeanVsApvIdx", 288, -0.5, 287.5);
  hNVsApvIdx = new TH1F("hNVsApvIdx", "hNVsApvIdx", 288, -0.5, 287.5);
  for(int rdoIdx=1; rdoIdx<=totRdo; rdoIdx++){
    for(int armIdx=0; armIdx<ArmPerRdo; armIdx++){
      for(int refApvIdx=0; refApvIdx<ApvPerArm; refApvIdx++){
        int glbElecApvIdx = refApvIdx + armIdx*ApvPerArm + (rdoIdx-1)*ApvPerRdo;        // 0-287
	  ...
	  for(int itb = 0 ; itb < 3 ; itb++){
            entries[itb] = hMaxTimeBinContents_APV.maxTimeBinArray_APV[glbElecApvIdx]->GetBinContent(itb+numTb/2);
            double fraction_tb = entries[itb]/entriesTB_all;
            hFracVsApvIdx[itb]->SetBinContent(glbElecApvIdx+1,fraction_tb);
            double errfrac = TMath::Sqrt(entries[itb]*(1.-2.*fraction_tb)+fraction_tb*fraction_tb*entriesTB_all)/entriesTB_all;
            hFracVsApvIdx[itb]->SetBinError(glbElecApvIdx+1, errfrac);
          }
	}
  ...
  fTBFrac = new TFile("test.root", "recreate");
  fTBFrac->cd();
  for(int i = 0 ; i < 3 ; i++) hFracVsApvIdx[i]->Write();
  hMeanVsApvIdx->Write();
  hNVsApvIdx->Write();
  fTBFrac->Close();
